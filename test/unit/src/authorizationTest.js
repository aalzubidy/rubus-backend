...
describe('authorizationSrc.js', function () {
  ...
  describe('register', () => {
    it('should return new user id', async () => {
      td.when(db.query(anything, anything)).thenReturn({ rows: [{ id: 1 }] });
      try {
        const input = {
          "name": "test user",
          "email": "test@test.com",
        ...
        const results = await authorizationSrc.register({ body: input });
    ...
    it('should return error on missing email and password on registration', async () => {
      try {
        const results = await authorizationSrc.register({ body: {} });
  ...
  describe('login', () => {
    it('should return error on missing email and password on login', async () => {
      try {
        const results = await authorizationSrc.login({ body: {} });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
        expect(JSON.stringify(error)).to.contain('Please provide email and password');
    ...
    it('should return unauthorized if username not in database', async () => {
      ...
      try {
        ...
        const results = await authorizationSrc.login({ body: input });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should return unauthorized if password does not match', async () => {
      td.when(db.query(anything, anything)).thenReturn({ rows: [{ id: 'test', email: 'test', name: 'test', password: 'test' }] });
      try {
        const input = {
          "email": "test@test.com",
          "password": "test1"
    ...
    it('should return access token and refresh token on successful login', async () => {
      td.when(db.query(anything, anything)).thenReturn({ rows: [{ id: 1, email: 'test@test.com', name: 'test', password: '$2b$12$r6vMkpEn/6nY.UvxrNLA.Ouu0jnCAm4PDNUBiDqUMeBstK/pyDsNm' }] });
      try {
        const input = {
          "email": "test@test.com",
          "password": "passwd"
  ...
  describe('logout', () => {
    it('should return error on missing token or refres token on logout', async () => {
      try {
        const results = await authorizationSrc.logout({ headers: {}, body: {} });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should return error on jwt verify', async () => {
      try {
        const results = await authorizationSrc.logout({ headers: { token: 'test' }, body: { refreshToken: 'test' } });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should return error on jwt id verify', async () => {
      ...
      stub1.onCall(0).returns({ id: 'test' });
      stub1.onCall(1).returns({ id: 'test1' });
      try {
        const results = await authorizationSrc.logout({ headers: { token: 'test' }, body: { refreshToken: 'test' } });
        expect(results).to.be.null;
      } catch (error) {
    ...
    it('should return error on deleting refresh token', async () => {
      let stub1 = sinon.stub(jwt, 'verify');
      stub1.onCall(0).returns({ id: 'test' });
      stub1.onCall(1).returns({ id: 'test' });
      td.when(db.query(anything, anything)).thenReject('fake error');
      try {
        const results = await authorizationSrc.logout({ headers: { token: 'test' }, body: { refreshToken: 'test' } });
        expect(results).to.be.null;
  ...
  describe('renewToken', () => {
    it('should return error on missing token or refres token on renew token', async () => {
      try {
        const results = await authorizationSrc.renewToken({ headers: {}, body: {} });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
        expect(JSON.stringify(error)).to.contain('Please provide token and refresh token');
    ...
    it('should return error on jwt id verify', async () => {
      let stub1 = sinon.stub(jwt, 'verify');
      stub1.onCall(0).returns({ email: 'test' });
      stub1.onCall(1).returns({ email: 'test1' });
      try {
        const results = await authorizationSrc.renewToken({ headers: { token: 'test' }, body: { refreshToken: 'test' } });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should return error on jwt verify', async () => {
      let stub1 = sinon.stub(jwt, 'verify');
      stub1.onCall(0).throws({ email: 'test' });
      try {
        const results = await authorizationSrc.renewToken({ headers: { token: 'test' }, body: { refreshToken: 'test' } });
        expect(results).to.be.null;
      } catch (error) {
    ...
    it('should renew token', async () => {
      ...
      stub1.onCall(0).returns({ id: 'test', name: 'test', email: 'test' });
      stub1.onCall(1).returns({ id: 'test', name: 'test', email: 'test' });
      td.when(db.query(anything, anything)).thenReturn({ rows: [{ email: 'test' }] });
  ...
  describe('verifyToken', () => {
    it('should return error on missing token', async () => {
      try {
        const results = await authorizationSrc.verifyToken({ headers: {} });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should return error on verify token', async () => {
      try {
        const results = await authorizationSrc.verifyToken({ headers: { token: 'test' } });
        expect(results).to.be.null;
      } catch (error) {
        expect(error).to.be.not.null;
    ...
    it('should verify token', async () => {
      let stub = sinon.stub(jwt, "verify").returns({ id: 'test' });
      try {
        const results = await authorizationSrc.verifyToken({ headers: { token: 'test' } });
        expect(results).to.deep.equal({ id: 'test' });
        stub.restore();
      } catch (error) {
        console.log(error);
        expect(error).to.be.null;
